...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:fb="http://www.facebook.com/2008/fbml" xmlns:og="http://opengraphprotocol.org/schema/" lang="{% for language in languages %}{% if language.active %}{{ language.lang }}{% endif %}{% endfor %}">
    <head>
        ...
        {# Estilos das Páginas Customizadas (Primeiro Sim, Ressignificação, Seja Ourives, Joias Exclusivas) #}
        {{ 'css/custom-pages-v26.css' | static_url | css_tag }}
    ...
    {% set store_body_slug %}
        ...
        {% elseif template == 'page' %}
            page--{{ page.name | replace(" ", "_") | lower }}
        ...
        {% elseif template == 'account.order' %}
            {{ 'Orden {1}' | translate(order.number) | lower }}
        {% else %}
            {% for crumb in breadcrumbs %}
                {% if crumb.last %}
                  page--{{ crumb.name | replace(" ", "_") | lower }}
                {% endif %}
            {% endfor %}
        {% endif %}
        {% endspaceless %}
    ...
    <body class="{% if customer %}customer-logged-in{% endif %} template-{{ template | replace('.', '-') }} transition-long {{ store_body_slug }}">
