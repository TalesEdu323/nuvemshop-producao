...
<div id="{{ modal_id }}" class="js-modal {% if modal_mobile_full_screen %}js-fullscreen-modal{% endif %} {% if desktop_overlay_only %}js-modal-overlay-md{% endif %} modal modal-{{ modal_class }} modal-{{modal_position}} modal-{{modal_position_desktop}}-md transition-{{modal_transition}} modal-{{modal_width}} transition-soft {% if modal_zindex_top %}modal-zindex-top{% endif %}" style="display: none;" {% if data_component %}data-component="{{ data_component }}"{% endif %}>
    ...
    {% endif %}
            <div class="{% if not (search_modal or modal_no_close) %}js-modal-close{% endif %} {% if modal_mobile_full_screen %}js-fullscreen-modal-close{% endif %} {% if modal_zindex_top %}js-close-over-modal{% endif %} modal-header {% if not modal_header_title %}modal-header-no-title modal-close{% endif %} {% if modal_close_floating and not modal_header_title %}modal-close-floating{% endif %} {{ modal_header_class }}">
                {% if modal_header_title %}
                    <div class="row no-gutters align-items-center">
                        <div class="col px-3 {{ modal_title_class }}">
    ...
    {% endif %}
    {% if (languages | length > 1) and modal_show_languages %}
