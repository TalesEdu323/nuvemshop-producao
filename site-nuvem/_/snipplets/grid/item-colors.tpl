{% if product.variations %}
    ...
    {% for variation in product.variations %}
        <div class="js-color-variant-available-{{ loop.index }} {% if variation.name|lower in ['color', 'cor'] %}js-color-variant-active {% if settings.item_colors_on_image == "image" %}item-colors-container{% endif %}{% endif %}" data-value="variation_{{ loop.index }}" data-option="{{ loop.index0 }}" >
            {% if variation.name|lower in ['color', 'cor'] %}
                {% if variation.options | length > 1 %}
                    <div class="item-colors">
                        {% set custom_options = variation.options | filter(o => o.custom_data) %}

                        {% if custom_options > 0 %}
                            {% for option in custom_options | slice(0,3) %}
                        ...
                        {% elseif custom_color_variants %}
                            <span class="item-colors-bullet item-colors-bullet-more w-auto" title="{{ 'Ver más colores' | translate }}">{{ custom_color_variants }} {{ 'colores' | translate }}</span>
                        {% endif %}
                    </div>
                {% endif %}
            {% endif %}
