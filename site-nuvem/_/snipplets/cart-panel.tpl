...
<div class="js-empty-ajax-cart cart-row text-center" {% if cart.items_count > 0 %}style="display:none;"{% endif %}>
 	{# Cart panel empty #}
    <div class="alert alert-info" data-component="cart.empty-message">{{ "El carrito de compras está vacío." | translate }}</div>
    <a href="{{ store.products_url }}" class="btn btn-default btn-medium btn-bold d-inline-block">{{ "Ver productos" | translate }}</a>
</div>
