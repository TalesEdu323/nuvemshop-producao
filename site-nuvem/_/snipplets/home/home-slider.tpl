...
{% if not mobile %}
<div class="js-home-main-slider-container {% if not settings.slider_full %}container{% endif %} {% if not has_main_slider %}hidden{% endif %}" data-transition="fade-in-up">
{% endif %}
	<div class="{% if mobile %}js-home-mobile-slider  {% if not settings.slider_full %}container{% endif %} {% else %}js-home-main-slider{% endif %}-visibility {% if has_main_slider and has_mobile_slider %}{% if mobile %}d-md-none{% else %}d-none d-md-block{% endif %}{% elseif not settings.toggle_slider_mobile and mobile %}hidden{% endif %}">
		<div class="section-slider position-relative">
			<div class="js-home-slider{% if mobile %}-mobile{% endif %} nube-slider-home swiper {% if not settings.slider_full %}theme-round{% endif %}">
							...
							<div class="slider-slide">
								...
								{% if has_text %}
									<div class="swiper-text{% if settings.slider_align == 'center' %} swiper-text-centered{% endif %} swiper-text-{{ slide.color }}">
										{% if slide.title %}
											<div class="swiper-title h1-huge mb-3">{{ slide.title }}</div>
										{% endif %}
										{% if slide.description %}
											<p class="swiper-description mb-3">{{ slide.description }}</p>
										{% endif %}
                ...
                <div class="swiper-arrows d-none d-md-block">
                    <div class="js-swiper-home-control js-swiper-home-prev{% if mobile %}-mobile{% endif %} swiper-button-prev d-inline-block">
						<svg class="icon-inline icon-lg icon-flip-horizontal"><use xlink:href="#chevron"/></svg>
					</div>
                    <div class="js-swiper-home-control js-swiper-home-next{% if mobile %}-mobile{% endif %} swiper-button-next d-inline-block svg-icon-invert ml-2">
						<svg class="icon-inline icon-lg"><use xlink:href="#chevron"/></svg>
					</div>
                </div>
			</div>
            <div class="js-swiper-home-pagination{% if mobile %}-mobile{% endif %} swiper-pagination swiper-pagination-bullets">
