{% if settings.head_background and 'head_background.jpg' | has_custom_image %}
    <link rel="preload" as="image" href="{{ 'head_background.jpg' | static_url | settings_image_url('1080p') }}" media="(min-width: 768px)">
{% endif %}

{% if settings.head_background_mobile and 'head_background_mobile.jpg' | has_custom_image %}
    <link rel="preload" as="image" href="{{ 'head_background_mobile.jpg' | static_url | settings_image_url('480w') }}" media="(max-width: 767px)">
{% endif %}
...
{% elseif template == 'category' %}
	...
	{% set has_general_banner = "banner-products.jpg" | has_custom_image %}
	{% set has_mobile_banner = "banner-products-mobile.jpg" | has_custom_image %}
	{% set category_banner = has_category_images or has_general_banner or has_mobile_banner %}
	...
	{% if category_banner %}
        ...
        {% set use_general_banner = has_general_banner and not has_category_images %}
		...
		{% if use_mobile_banner %}
			{% set mobile_image_sizes = ['large', 'huge'] %}
			{% set category_mobile_images = [] %}

			{% for size in mobile_image_sizes %}
				{# Define images for general mobile banner #}
				{% set category_mobile_images = category_mobile_images|merge({(size):('banner-products-mobile.jpg' | static_url | settings_image_url(size))}) %}
			{% endfor %}

			<link rel="preload" fetchpriority="high" as="image" href="{{ category_mobile_images['large'] }}" imagesrcset="{{ category_mobile_images['large'] }} 480w, {{ category_mobile_images['huge'] }} 640w" media="(max-width: 767px)">
		{% endif %}

        {% if has_category_images or use_general_banner %}
            <link rel="preload" fetchpriority="high" as="image" href="{{ category_images['large'] }}" imagesrcset="{{ category_images['large'] }} 480w, {{ category_images['huge'] }} 640w, {{ category_images['original'] }} 1024w, {{ category_images['1080p'] }} 1920w"{% if use_mobile_banner %} media="(min-width: 768px)"{% endif %}>
        {% endif %}
